...
class Validator
{
    ...
    protected function validateMimes($attribute, $value, $parameters)
    {
        ...
        if ($value['tmp_name'] != '') {
            return in_array(pathinfo($value['name'], PATHINFO_EXTENSION), $parameters);
        } else {
            return false;
        }
    }
    ...
    {
        if ($extensions) {
            $keys = array_map(array($this, 'snakeCase'), array_keys($extensions));
